...
import { Search, ArrowRight, Eye, Tag, Hash, Calendar } from 'lucide-react';
...
const Posts: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-32 min-h-screen text-slate-100">
                ...
                {filteredPosts.map((post, idx) => {
                    ...
                    return (
                        <motion.div 
                            ...
                            className="group bg-brand-surface/20 backdrop-blur-xl rounded-[2.5rem] overflow-hidden border border-slate-800/60 hover:border-brand-accent/40 transition-all duration-500 flex flex-col h-full hover:shadow-[0_20px_50px_rgba(0,0,0,0.4),0_0_30px_rgba(34,211,238,0.1)] hover:-translate-y-2 relative"
                        >
                            {/* Visual Layer: Inner boundary glow */}
                            <div className="absolute -inset-px bg-gradient-to-br from-brand-accent/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none rounded-[2.5rem]"></div>

                            <Link to={`/posts/${post.id}`} className="block h-60 overflow-hidden relative">
                                <img src={post.imageUrl || 'https://images.unsplash.com/photo-1618477247222-acbdb0e159b3'} alt={safeTitle} className="w-full h-full object-cover grayscale brightness-[0.4] group-hover:grayscale-0 group-hover:brightness-100 group-hover:scale-105 transition-all duration-1000" />
                                <div className="absolute inset-0 bg-gradient-to-t from-brand-darker/90 via-transparent to-transparent opacity-80" />
                                <div className="absolute top-6 left-6">
                                    <span className="bg-brand-accent/90 backdrop-blur-md text-brand-darker text-[10px] font-black px-4 py-2 rounded-xl uppercase tracking-[0.2em] border border-white/20 shadow-xl">
                                        {safeCategory}
                                    </span>
                                </div>
                            </Link>
                            <div className="p-8 flex flex-col flex-grow relative z-10">
                                <h3 className="text-2xl font-black text-white mb-4 group-hover:text-brand-accent transition-colors italic tracking-tight leading-[1.2]">
                                    <Link to={`/posts/${post.id}`}>{safeTitle}</Link>
                                </h3>
                                
                                <div className="flex flex-wrap gap-2 mb-8">
                                    {tags.slice(0, 3).map((tag, i) => (
                                        <span key={i} className="text-[10px] font-black uppercase tracking-widest bg-slate-900/60 text-slate-400 px-3 py-1.5 rounded-lg border border-slate-800 flex items-center gap-2 hover:border-brand-accent/30 hover:text-slate-200 transition-all">
                                ...
                                <div className="mt-auto flex items-center justify-between pt-6 border-t border-slate-800/50">
                                    <div className="flex items-center gap-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                        <span className="flex items-center gap-1.5 bg-brand-darker/40 px-3 py-1.5 rounded-lg border border-slate-800/50">
                                    ...
                                    <Link to={`/posts/${post.id}`} className="flex items-center gap-2 text-[10px] font-black text-white uppercase tracking-[0.2em] group-hover:text-brand-accent transition-all">
                                        DECRYPT 

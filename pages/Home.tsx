...
import { ArrowRight, Zap, Code, Cpu, Shield, Globe, Eye, Calendar } from 'lucide-react';
                ...
                {featuredPosts.map((post, idx) => {
                    ...
                    return (
                        <motion.div 
                            ...
                            className="group flex flex-col h-full bg-brand-surface/40 backdrop-blur-md rounded-[2rem] overflow-hidden border border-slate-800/60 hover:border-brand-accent/50 transition-all duration-500 hover:shadow-[0_0_40px_-10px_rgba(34,211,238,0.2)] hover:-translate-y-2"
                        >
                            <div className="relative h-64 overflow-hidden">
                                <img src={post.imageUrl} alt={safeTitle} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000" />
                                <div className="absolute inset-0 bg-gradient-to-t from-brand-darker/80 via-transparent to-transparent" />
                                <div className="absolute bottom-6 left-6">
                                    <span className="bg-brand-accent text-brand-darker text-[9px] font-black px-4 py-1.5 rounded-lg uppercase tracking-widest shadow-xl">
                                        {safeCategory}
                                    </span>
                                </div>
                            </div>
                            <div className="p-10 flex flex-col flex-grow">
                                <h3 className="text-2xl font-black text-white mb-4 italic tracking-tight group-hover:text-brand-accent transition-colors">
                                    {safeTitle}
                                </h3>
                                <p className="text-slate-500 text-sm mb-10 line-clamp-2 italic font-medium">
                                ...
                                <div className="mt-auto flex items-center justify-between pt-6 border-t border-slate-800/50">
                                    <div className="flex items-center gap-4 text-[10px] font-black text-slate-600 uppercase tracking-widest">
                                        ...
                                        <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {post.createdAt ? new Date(post.createdAt).toLocaleDateString() : 'N/A'}</span>
                                    </div>
                                    <Link to={`/posts/${post.id}`} className="text-brand-accent opacity-0 group-hover:opacity-100 transition-opacity">

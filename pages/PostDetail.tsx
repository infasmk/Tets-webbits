  ...
  return (
    <article className="min-h-screen pb-20 bg-brand-darker overflow-x-hidden">
        <div className="h-[60vh] md:h-[70vh] relative w-full overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 w-full z-20 p-6 sm:p-12 lg:p-20">
                <div className="max-w-5xl mx-auto">
                    <Link to="/posts" className="inline-flex items-center text-slate-400 hover:text-brand-accent mb-8 transition-all font-black uppercase tracking-widest text-[9px] bg-brand-darker/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/5">
                        <ArrowLeft className="w-4 h-4 mr-2" /> Back to Knowledge Base
                    </Link>
                    <div className="flex gap-3 mb-6">
                        <span className="bg-brand-accent text-brand-darker font-black px-5 py-2 rounded-xl text-[10px] uppercase tracking-widest">
                            {safeCategory}
                        </span>
                    </div>
                    <h1 className="text-4xl sm:text-6xl md:text-8xl font-black text-white mb-10 leading-[0.9] tracking-tighter italic break-words">
                        {safeTitle}
                    </h1>
                </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 md:-mt-32 relative z-30">
            <div className="bg-brand-surface border border-slate-800/80 rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-20 shadow-2xl overflow-hidden">
                ...
                <div className="mt-16 pt-12 border-t border-slate-800/50 flex flex-wrap gap-2 md:gap-3 mb-10">
                    {(Array.isArray(post.tags) ? post.tags : []).map((tag, idx) => (
                        <div key={idx} className="flex items-center gap-2 bg-brand-dark/50 px-4 py-2 md:px-5 md:py-3 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-500 border border-slate-800">
                            <Tag className="w-3 h-3 md:w-3.5 md:h-3.5 text-brand-accent" /> {getSafeStr(tag)}
                        </div>
                    ))}
                </div>

                <div className="mt-16 md:mt-24 bg-brand-dark/60 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 border border-slate-800 flex flex-col lg:flex-row justify-between items-center gap-10 md:gap-12">
                    <div className="text-center lg:text-left flex-1">
                        <h3 className="text-2xl md:text-4xl font-black text-white mb-4 italic tracking-tight uppercase leading-none break-words">Project Resources</h3>
                        <p className="text-slate-500 font-medium max-w-md mx-auto lg:mx-0 text-sm md:text-base">Access production source code or request a specific implementation guide.</p>
                    </div>
                    
                    <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-4 md:gap-5 w-full lg:w-auto">
                        {hasDownload && (
                            <a 
                                href={ensureAbsoluteUrl(getSafeStr(post.downloadUrl))} 
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center justify-center gap-3 px-8 md:px-12 py-5 md:py-6 bg-white text-brand-darker hover:bg-slate-200 rounded-2xl md:rounded-[2rem] font-black transition-all uppercase tracking-widest text-[10px] md:text-xs active:scale-95 shadow-lg"
                            >
                        ...
                        <a 
                            ...
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="flex items-center justify-center gap-4 px-8 md:px-12 py-5 md:py-6 bg-[#25D366] text-white hover:bg-[#22c35e] rounded-2xl md:rounded-[2rem] font-black transition-all shadow-lg uppercase tracking-widest text-[10px] md:text-xs active:scale-95"
                        >
                            Request Custom
                        </a>
                        {hasCustomBtn && (
                            <a 
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center justify-center gap-4 px-8 md:px-12 py-5 md:py-6 bg-brand-accent text-brand-darker hover:bg-brand-accentHover rounded-2xl md:rounded-[2rem] font-black transition-all uppercase tracking-widest text-[10px] md:text-xs active:scale-95 shadow-lg shadow-brand-accent/10"
                            >
